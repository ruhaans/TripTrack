...
# Seat helpers are robust to whether you added model properties or not.
def _seats_taken(trip: Trip) -> int:
    try:
        # fast if related_name="registrations"
...
def home(request):
    """Public landing showing the current active trip."""
...
def register(request):
    ...
    existing = Registration.objects.filter(trip=trip, user=request.user).first()
    if existing:
    ...
    if request.method == "POST":
        ...
        initial = {
    "full_name": "",     # force user to enter real name
        # prefill for India
}
...
def trip_details(request):
    ...
    if not trip:
        ...
        return redirect("trips:home")
    return render(request, "trips/details.html", {"trip": trip, "title": f"{trip.name} · Details"})
...
from .views import _active_trip, _is_staff  # if these are in this file already, keep one definition
...
def headcount(request):
    ...
    only = request.GET.get("only") or ""  # "", "unchecked"

    # ----- base queryset -----
    regs_qs = Registration.objects.filter(trip=trip).select_related("user")
    ...
    if only == "unchecked":
        if mode == "out":
            regs_qs = regs_qs.filter(boarded_outbound=False)
        else:
            regs_qs = regs_qs.filter(boarded_return=False)
    ...
    # totals (overall)
    all_qs = Registration.objects.filter(trip=trip)
    totals_all = {
        "total": all_qs.count(),
    ...
    totals_shown = {
        "total": len(regs),
        "checked": count_checked_visible(regs),
    }
    ...
    if request.method == "POST":
        ...
        for r in regs:  # only rows being viewed/filtered
    ...
    # --- mode selection: 'out' for outbound, 'ret' for return ---
    mode = (request.GET.get("mode") or "out").lower()
    if mode not in ("out", "ret"):
        mode = "out"

    # --- search + sort (applies to the visible set) ---
    q = (request.GET.get("q") or "").strip()
    order = request.GET.get("order") or "name_asc"
    ...
    # Visible totals for current mode
    def count_checked_visible(rs):
        if mode == "out":
            return sum(1 for r in rs if r.boarded_outbound)
        return sum(1 for r in rs if r.boarded_return)
    ...
    if request.method == "POST":
        ...
        for r in regs:  # only update rows the organizer is currently viewing
            key = f"{'out' if mode=='out' else 'ret'}-{r.id}"
            new_val = (request.POST.get(key) == "on")
        ...
        # Preserve mode / filters on redirect
        params = []
        if mode:
...
def manage_dashboard(request):
    ...
    if trip:
        ...
        total = q.count()
        paid = q.filter(status="paid").count()
        pending = q.filter(status="pending").count()
        seats_left = _seats_left(trip)
        capacity = trip.capacity or 0
        filled_pct = 0
        if capacity > 0:
            filled = min(total, capacity)
            filled_pct = round((filled / capacity) * 100)
    ...
    return render(
        ...
        {"trip": trip, "stats": stats, "recent_regs": recent_regs, "title": "Organizer"},
    )
...
def trip_list(request):
    ...
    return render(request, "manage/trip_list.html", {
        "trips": trips,
        "title": "Trips",
    })
...
def trip_set_active(request, trip_id: int):
    ...
    trip.is_active = True
    trip.save()  # if your model ensures single-active, others will be deactivated there
...
def manage_regs(request):
    ...
    paid = regs_qs.filter(status="paid").count()
    pending = regs_qs.filter(status="pending").count()
    cancelled = regs_qs.filter(status="cancelled").count() if hasattr(Registration, "status") else 0
...
def export_regs_csv(request):
    trip = _active_trip()
    if not trip:
        return HttpResponse("No active trip", status=400)

    regs = Registration.objects.filter(trip=trip).order_by("full_name")
    ...
    writer.writerow([
  "Full Name","Email","Phone","DOB","Park Choice","Status",
  "Imagica Transaction","Price","Gift Code","Outbound","Return","Created At"
])
    for r in regs: 
        writer.writerow([
            r.full_name, r.user.email, r.phone, r.dob, r.get_park_choice_display(),
            r.status, r.imagica_transaction, r.price or "", r.gift_code or "",
            r.boarded_outbound, r.boarded_return, r.created_at
            ])
    return resp
...
def trip_edit(request, trip_id: int):
    ...
    return render(request, "manage/trip_form.html", {"form": form, "title": f"Edit Trip · {trip.name}"})
